...
cfg.CONF.register_group(cfg.OptGroup(name='app',
                                     title='App options'))
cfg.CONF.register_opts([cfg.StrOpt('keystone_url',
                                   default=None,
                                   help='Keystone API URL'),
                        cfg.StrOpt('tripleo_api_url',
                                   default=None,
                                   help='TripleO API URL'),
                        cfg.StrOpt('validations_url',
                                   default=None,
...
@app.route('/js/rdo_director_ui_config.js')
def send_config_js():
    return send_from_directory(LOCAL_STATIC_PATH, 'rdo_director_ui_config.js',
                               mimetype=mimetypes.guess_type('rdo_director_ui_config.js')[0])
...
def create_config_js(app_conf):
    ...
    json_str = json.dumps({key: val for key, val in app_conf.items() if val is not None}) 
    file_path = os.path.join(LOCAL_STATIC_PATH, 'rdo_director_ui_config.js')
    with open(file_path, 'w') as js:
        js.write("window.rdoDirectorUiConfig = %s;" % json_str)
